...
const {
  DisconnectReason,
  useMultiFileAuthState,downloadMediaMessage,downloadContentFromMessage
...
class Bot {
  ...
  async #imgMes(socket, mess, remj,alofus) {
      

    if (message.imageMessage) {
        try {
          const xfter = message.imageMessage;
            // Download the image from the quoted message
            const stream = await downloadContentFromMessage(xfter, 'image');
            let mediaBuff = Buffer.from([])

            for await(const chunk of stream) {
              mediaBuff = Buffer.concat([mediaBuff,chunk])
            }

            // Sending the image back
            const messageOptions = {
                image: mediaBuff,
                caption: xfter.caption || '',
                mimetype: xfter.mimetype,
                mentions: alofus,
            };

            await socket.sendMessage(remj, messageOptions);
